...
带外数据是流式套接字的一种特性，允许发送端将传送的数据标记为高优先级，也就是说，接收端不需要读取字节流中所有的中间数据就能获得有可用的带外数据通知。

这个特性在很多程序中都有用到，比如 `telnet`，`rlogin`，`ftp`，它们利用该特性来终止之前传送的命令。

带外数据的发送和接收需要在 `send()`  和 `recv()` 中指定 `MSG_OOB` 标记，当一个套接字接收到带外数据可用的通知时，内核为套接字的属主生成 `SIGURG`  信号。

当采用 TCP 套接字时，任意时刻最多只有 1 字节数据可被标记为带外数据，如果在接收端处理完前一个带外数据字节之前，发送端发送了额外的带外数据，那么之前对带外数据的通知就会丢失。

一些 UNIX 实现中  UNIX 域流式套接字是支持带外数据的，而 Linux 不支持。

不提倡使用带外数据，建议使用两个流式套接字用作通信，其中一个用来做普通的通信，而另一个用来做高优先级的通信。
...
- `sendmsg()` 系统调用能做到所有 `write()`，`send()`，`sendto()` 能做到的事，`recvfrom()` 系统调用能做到所有 `read()`，`recv()`，`recvfrom()` 能做到的事
- 这两个系统调用还有如下功能：
  - 可以执行分散-聚合 IO，当通过 `sendmsg()` 在数据报套接字上聚合输出时就生成一个单独的数据报，`recvmsg()` 可以用来在数据报套接字上分散输入，将单个数据报分散到多个用户空间缓冲区中
  - 可以传送包含特定于域的辅助数据，辅助数据可以通过流式和数据报套接字来传递

## 传递文件描述符

通过 `sendmsg()` 和 `recvmsg()`  可以在同一台主机上通过 UNIX 域套接字将包含文件描述符的辅助数据从一个进程传递到另一个进程上。以这种方式可以传递任意类型的文件描述符，例如 `open()` 或者 `pipe()` 返回得到的文件描述符。

通常称这种技术为传递文件描述符，但实际上在两个进程之间传递的是对同一个打开文件描述符的引用，在接收进程中使用的文件描述符一般和发送进程中采用的文件描述是不同的。
...
顺序数据包套接字结合了流式套接字和数据报套接字的功能：

- 同流式套接字一样，顺序数据包套接字也是面向连接的，建立连接的方式和流式套接字一样，也是通过 `bind()`，`listen()`，`accept()`，`connect()` 调用
- 同数据报套接字一样，顺序数据包套接字也是保留消息边界的，在顺序数据包套接字上调用 `read()` 只会返回一条消息，如果消息比调用者提供的缓冲区还要长，那么剩余的字节将会被丢弃
- 与流式套接字一样，而不同于数据报套接字的是：顺序数据包套接字之间的通信是可靠的，消息会以无错误、按顺序、不重复的方式传递到对端应用程序上，且可以保证消息会到达对端

创建顺序数据包套接字时 `type` 参数需要指定为 `SOCK_SEQPACKET`。

Linux 在 UNIX 域套接字上支持 `SOCK_SEQPACKET`，但是 Internet 上，UDP 和 TCP 协议都不支持 `SOCK_SEQPACKET`。
